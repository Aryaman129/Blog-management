...
async function main() {
    ...
    for (const tagName of tags) {
        const tag = await prisma.tag.upsert({
            ...
            create: {
                name: tagName,
                slug: (0, helpers_1.generateSlug)(tagName),
            },
        });
...
Master these techniques and you'll have powerful tools for creating complex, responsive layouts!`,
    ...
    for (const postData of blogPostsData) {
        const slug = (0, helpers_1.generateSlug)(postData.title);
        const readTime = (0, helpers_1.calculateReadTime)(postData.content);
...
The application successfully handles 500+ concurrent users per room with sub-100ms latency for drawing operations.`,
    ...
    for (const projectData of projectsData) {
        const slug = (0, helpers_1.generateSlug)(projectData.title);
        const project = await prisma.project.create({
            data: {
                ...
                status: projectData.status.toUpperCase().replace('-', '_'),
